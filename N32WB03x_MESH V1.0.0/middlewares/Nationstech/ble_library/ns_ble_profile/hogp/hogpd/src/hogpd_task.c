...
void hogpd_task_init(struct ke_task_desc *p_task_desc)
{
    // Get the address of the environment
    struct hogpd_env_tag *p_hogpd_env = PRF_ENV_GET(HOGPD, hogpd);

    p_task_desc->msg_handler_tab = hogpd_msg_handler_tab;
    p_task_desc->msg_cnt         = ARRAY_LEN(hogpd_msg_handler_tab);
    p_task_desc->state           = p_hogpd_env->state;
    p_task_desc->idx_max         = HOGPD_IDX_MAX;
}
